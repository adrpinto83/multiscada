...
const RANGOS_CV = {
  y1: { min: 100, max: 280, unidad: '°C',  ok: v => v >= 160 && v <= 240 },
  y2: { min: 0,   max: 10,  unidad: '%',   ok: v => v <= 5 },
  y3: { min: 80,  max: 250, unidad: '°C',  ok: v => v <= 200 },
    ...
    <div className={`bg-navy-700 border rounded-xl overflow-hidden ${ok ? 'border-gray-700/50' : 'border-red-500/50'}`}>
      <div className={`flex items-center justify-between px-4 py-3 border-b ${
        ...
        <div className="flex items-center gap-2">
          <div>
            <div className="font-mono text-base text-amber-400 font-bold">{pid.tag}</div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          {[
            { etq: 'PV',       val: pid.pv,  color: emuCV ? '#c084fc' : ok ? '#f59e0b' : '#f87171' },
            { etq: 'Consigna', val: pid.sp,  color: '#fbbf24', clickable: true },
            { etq: 'Error',    val: error,   color: Math.abs(error) < 2 ? '#4ade80' : '#fbbf24', signo: true },
            ...
            <div key={etq} className="bg-navy-800/60 rounded-lg p-2">
              ...
              {clickable && editandoSP ? (
                ...
                >
                  {signo && val >= 0 ? '+' : ''}{val.toFixed(val < 10 && val >= 0 ? 2 : 1)}
                </div>
              )}
        ...
        <div>
          <div className="h-4 bg-navy-800 rounded-full relative overflow-hidden">
            <div className={`h-full rounded-full transition-all duration-300 ${emuCV ? 'bg-purple-500/40' : ok ? 'bg-amber-500/40' : 'bg-red-500/40'}`}
              style={{ width: `${pvPct}%` }} />
            ...
            <div className="absolute top-0 h-full flex items-center"
              ...
              <div className={`w-3 h-3 rounded-full border-2 ${emuCV ? 'border-purple-400 bg-purple-400/50' : ok ? 'border-amber-400 bg-amber-400/50' : 'border-red-400 bg-red-400/50'}`} />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-1">
          ...
          {!esModoAuto && !emuMV ? (
            ...
            <div className="h-3 bg-navy-800 rounded-full overflow-hidden">
              <div className={`h-full rounded-full transition-all duration-300 ${emuMV ? 'bg-purple-500/50' : 'bg-amber-500/40'}`}
        ...
        <div className="border-t border-gray-700/50 pt-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => setEditandoSint(!editandoSint)}
              className="text-xs font-label text-amber-400/70 hover:text-amber-400 transition-colors">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={k} className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-xs text-amber-400 w-10 text-right">{sint[k].toFixed(2)}</span>
                </div>
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-2 text-center">
              {[{ l: 'Kp', v: pid.Kp }, { l: 'Ki', v: pid.Ki }, { l: 'Kd', v: pid.Kd }].map(({ l, v }) => (
                <div key={l} className="bg-navy-800/40 rounded px-2 py-1">
                  ...
                  <div className="font-mono text-sm text-amber-400/80">{v.toFixed(3)}</div>
                </div>
              ))}
            </div>
          )}
...
export default function CafeControladores() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="px-6 py-3 border-b border-gray-700/50 bg-navy-800/50 shrink-0 flex items-center justify-between">
        <div>
          <h1 className="font-label text-lg font-bold text-white">Controladores PID — Planta Café</h1>
