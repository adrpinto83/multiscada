...
export default function TrendChart({ data, pvKey, spKey, mvKey, title, unit, pvColor = '#00d4ff', spColor = '#fbbf24', mvColor = '#a78bfa', yDomain, height = 180 }) {
  const tMin = data.length > 0 ? data[0].t : 0;
  const tMax = data.length > 0 ? data[data.length - 1].t : 120;
  ...
  return (
    <div className="bg-navy-700 border border-gray-700/50 rounded-lg overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-navy-800 border-b border-gray-700/50">
        <span className="font-label text-sm font-bold text-white tracking-wide">{title}</span>
        <div className="flex items-center gap-3 text-xs">
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-4 h-0.5 border-t-2 border-dashed border-yellow-400 inline-block" />
      ...
      <div style={{ height }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 4 }}>
            ...
            {/* SP reference line (dashed yellow) */}
            {spKey && (
              <Line
                type="monotone"
                dataKey={spKey}
